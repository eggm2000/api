...
export const postUsuario = async (req, res) => {
  try {
    ...
    const hashedPassword = await bcrypt.hash(usr_clave, salt); // Encripta contraseña

    const [rows] = await conmysql.query(
      'INSERT INTO usuarios (usr_usuario, usr_clave, usr_nombre, usr_telefono, usr_correo, usr_activo) VALUES (?, ?, ?, ?, ?, ?)',
      [usr_id, usr_usuario, hashedPassword, usr_nombre, usr_telefono, usr_correo, usr_activo]
    );
...
export const putUsuarios = async(req, res) => {
 ...
 export const patchUsuarios=     //PATCH MODIFICAR
 async(req, res)=>{
     try {
         const {id}=req.params
     ...
     const[result]=await conmysql.query('update usuarios set usr_usuario=IFNULL(?,usr_usuario),usr_clave=IFNULL(?,usr_clave),usr_nombre=IFNULL(?,usr_nombre),usr_telefono=IFNULL(?,usr_telefono),usr_correo=IFNULL(?,usr_correo),usr_activo=IFNULL(?,usr_activo) where usr_id=?',
         ...
         const[rows]=await conmysql.query('select * from usuario where usr_id=?',[id])
         res.json(rows[0])
     } catch (error) {
         return res.status(500).json({message:'Error en el servidor'})
     }
 ...
 export const deleteUsuarios= //DELETE ELIMINAR
 async(req, res)=>{
     try {
